...
While a tuple reflects a relationship between Object and Subject, they do not
completely define the effective ACLs.
...
Subject is either a concrete subject id or a subject set expanding to more
Subjects.
...
SubjectSet refers to all subjects which have the same `relation` to an `object`.
...
The request for a ExpandService.Expand rpc. Expands the given subject set.
...
ListRelationTuplesRequest_Query for more querying details.
...
- relation only: get all groups that have members; e.g. get all directories that
  have content
