...
indirections assemble an acyclic directed graph, known as [the graph of relations](concepts/graph-of-relations).
...
searching the graph starting at `object` going through `relation` trying to reach `user`. The
