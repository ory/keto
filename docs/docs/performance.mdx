...
In essence, the check-engine assumes that the relationtuples and their
indirections assemble an acyclic directed graph. Each node represents a subject,
and every child is a subject the parent subjectset can be expanded as.
...
file#owner@user1               // file owner record; indirectly gets access
...
searching the graph starting at `object#relation` trying to reach `user`. The
request is allowed iff there is such a path.
