...
application maps to the actual object metadata. For the sake of readability, the
code samples use the object path instead. Please refer to the
[objects page](../concepts/objects.mdx) for why the mapping is necessary.

### Displaying who has access

To assist users with managing permissions for their files, the application has a
view to display exactly who has access to a file and why. In this example, we
assume that the application knows the following files and directories:
...
also for what reason they were included. In many cases the application would not
want to list all subject IDs but rather abstract away the group e.g. the owners
