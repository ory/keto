---
title: Check whether a Subject has Access to Something
---
...
The request (here `decypher` of the message `02y_15_4w350m3`) is then translated into a
request to Ory Keto's [check API](/TODO). Basically, the application is asking
Keto "Is john allowed to decypher the text 02y_15_4w350m3?" This is encoded as
the following [relation tuple](../concepts/relation-tuples):
...
messages are stored in Ory Keto and described by a `decypher` relation.

### Directly Defined Access

Ory Keto can know the exact relation tuple that is checked for. Intuitively,
this means that `john` was allowed to `decypher` the message `02y_15_4w350m3`
directly (imagine a "Share with `john`" input in a UI).

Try this yourself by first adding the relation tuple using the [write API](/TODO):
...
```keto-relation-tuples
02y_15_4w350m3#decypher@(hackers#member)
```
