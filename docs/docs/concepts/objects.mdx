...
The application will have to translate an incoming request setting the value to
the corresponding range the value is in. It is important to understand that Ory
Keto does not know how to interpret the `greater_than` and `smaller_equal`
information. Rather, the application has to preprocess and transcode the value
into the corresponding range representation.
